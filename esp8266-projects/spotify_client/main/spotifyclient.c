...
esp_err_t spotify_client_init(UBaseType_t priority, QueueHandle_t *playing_q_hlr) {
    ...
    if (state.access_token == NULL) return ESP_FAIL;

    state.client = esp_http_client_init(&config);
    if (state.client == NULL) return ESP_FAIL;
    ...
    if (client_lock == NULL) return ESP_FAIL;
...
static esp_err_t _validate_token() {
    ...
    if ((tokens->expiresIn - 10) > time(0)) return ESP_OK;
    ...
    retry:;
        ...
        if (state.err == ESP_OK) {
            ...
            if (PLAYBACK_TRANSFERED(state)) {
                ESP_LOGI(TAG, "Playback transfered to: %s", track->device->id);  // posible crash here
